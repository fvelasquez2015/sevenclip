...
<head>
	...
	<link href="horario_estilo.css" rel="stylesheet" type="text/css">
	<script type="text/javascript" src="https://code.jquery.com/jquery-2.1.3.min.js"></script>
	<script type="text/javascript" src="//maxcdn.bootstrapcdn.com/bootstrap/3.3.2/js/bootstrap.min.js"></script>
	<script type="text/javascript" src="horario_script.js"></script>
	<script>
		if (window.XMLHttpRequest) objAjax = new XMLHttpRequest() 								// Mozilla
		else if (window.ActiveXObject) objAjax = new ActiveXObject("Microsoft.XMLHTTP")	        // Explorer
		...
		function EliminaDato(codigo,dato){
			var mensaje = confirm("Deseas eliminar esta asignatura del horario?");
			if (mensaje) {
				objAjax.open("POST","lista.php");
				objAjax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
		...
		function MostrarDatos(){
			if(objAjax.readyState == 4){
				document.getElementById("tabla").innerHTML = objAjax.responseText;
			}
		}
	</script>
<body>
	<button onclick="TraerDatos(1,0)">Mostrar asignaturas inscritas</button>
	<div id="tabla">
...
<?php
	...
	$mysqli = new mysqli('db.inf.uct.cl', 'fvelasquez', '123456', 'fvelasquez'); // Base de datos = fvelasquez
	...
	if(isset($_GET['add-event']))
	{
		...
		else
		{
				...
				VALUES (?, ?, ?, ?)"))
			{
				...
				header('location: horario_index.php');
			}
			else
			{
				die($mysqli->error . '
	...
	$result = $mysqli->query('SELECT * FROM seven_horario');
